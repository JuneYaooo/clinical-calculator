...
| 判断是否可计算 | “这个评分在 skill 里能直接计算吗？如果不能，请告诉我缺少什么来源。” |
...
- 条目只有名称、但缺少公式或授权内容时，Agent 会明确说明无法计算的原因。
...
| 内容状态 | 数量 | Agent 会如何处理 |
| --- | ---: | --- |
| 可直接计算 | **643** | 核对输入后计算并解释 |
| 待补原始来源 | **110** | 说明缺少的公式、系数、表格或版本 |
| 指南与知识路径 | **206** | 作为知识路由，不伪装成单一公式 |
| 受控内容 | **95** | 说明授权或版本限制，不擅自复刻 |

目录共收录 **1,054** 个有效逻辑条目，覆盖 **981** 个唯一中文名称。可直接计算表示本地具备明确输入契约与计算逻辑，不等同于已经获得临床发布批准。
...
- 若条目无法可靠运行，Agent 应说明状态和所需材料，而不是生成看似可信的结果。
